...
func _move_note(delta: float) -> void:
	if !isHoldActive:
		...
		position.x += speed * delta * -sin(angle)
	if isHoldActive:
		holdBodyContainer.position.y += speed * delta * cos(angle)
		holdBodyContainer.position.x += speed * delta * -sin(angle)
