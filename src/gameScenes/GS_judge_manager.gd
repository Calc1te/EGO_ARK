...
@export var referenceOffset_static : float = 0
...
func updateSpeed(spawnHeight):
	referenceOffset_static = spawnHeight*10/(globalSpeed*2)
	print("speed Updated, new reference offset: ",referenceOffset_static)
...
func calculate_acc(acc : int, holdError : int):
	...
	if acc == 1000 && holdError == -1:
		...
		return
	var hitError : int = abs(acc - referenceOffset_static + playerOffset)
	print(acc - referenceOffset_static + playerOffset)
	if holdError == -1:
		if hitError > DT_BAD:
			print("miss")
